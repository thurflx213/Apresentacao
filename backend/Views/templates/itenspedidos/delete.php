<div class="page-wrapper">
    ...
    <div class="form-card card-confirmacao">
        <p>Você tem certeza que deseja excluir este item do pedido?</p>
        <h3 class="item-to-delete">Item #<?= htmlspecialchars($itempedido['id_itens_pedidos']); ?> — Pedido #<?= htmlspecialchars($itempedido['id_pedido']); ?></h3>
        <p class="warning-message">Esta ação não pode ser desfeita. O item será removido permanentemente do pedido.</p>

        <hr>
        <form action="/backend/itenspedidos/deletar/<?= $itempedido['id_itens_pedidos']; ?>" method="POST">
            <input type="hidden" name="id_itens_pedidos" value="<?= $itempedido['id_itens_pedidos']; ?>">
            <div class="button-group">
                <button type="submit" class="btn-confirm-delete">
                    <i class="fa fa-trash"></i> Sim, Excluir Item
                </button>
                <a href="/backend/itenspedidos/listar" class="btn-cancelar">
