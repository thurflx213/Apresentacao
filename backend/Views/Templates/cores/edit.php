<div class="page-wrapper">
    <h3 class="page-title">
        ...
        Editando Cor: <span style="color: #f2cc7d;"><?= htmlspecialchars($cor['cor_cores']); ?></span>
    </h3>
    ...
    <form action="/backend/cor/atualizar/<?= $cor['id_cores']; ?>" method="post" class="form-card">
        <input type="hidden" name="id_cores" value="<?= $cor['id_cores']; ?>">

        <div class="form-group">
            <label><i class="fa fa-box"></i> ID do Produto:</label>
            <input type="number" name="id_produto" value="<?= $cor['id_produto']; ?>" required min="1">
        </div>

        <div class="form-group">
            <label><i class="fa fa-palette"></i> Cor:</label>
            <input type="text" name="cor_cores" value="<?= htmlspecialchars($cor['cor_cores']); ?>" required>
        </div>

        <div class="form-group">
            <label><i class="fa fa-cubes"></i> Quantidade em Estoque:</label>
            <input type="number" name="quantidade_cores" value="<?= $cor['quantidade_cores']; ?>" required min="0">
        </div>
