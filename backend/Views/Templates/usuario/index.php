...
<style>
    ...
    .btn-activate { background: rgba(40, 167, 69, 0.05); color: #28a745; border: 1px solid #28a745; }
    .btn-activate:hover { background: #28a745; color: #fff; }

    /* --- NOVO ESTILO DA PAGINAÇÃO (KOKETSU STYLE) --- */
...
<div class="page-wrapper">
    ...
    <main>
        <table class="user-table" id="userTable">
            ...
            <tbody id="tableBody">
                ...
                <tr class="user-row <?= $is_inativo ? 'tr-inativo' : '' ?>"> 
                    ...
                    <td style="text-align: center;">
                        <a href="/backend/usuario/editar/<?= $usuario['id_usuarios'] ?>" class="btn-action-small btn-edit"><i class="fa fa-pencil"></i> Editar</a>
                        <?php if ($is_inativo): ?>
                            <a href="/backend/usuario/ativar/<?= $usuario['id_usuarios'] ?>" class="btn-action-small btn-activate"><i class="fa fa-check"></i> Ativar</a>
                        <?php else: ?>
                            <a href="/backend/usuario/excluir/<?= $usuario['id_usuarios'] ?>" class="btn-action-small btn-toggle" onclick="return confirm('Inativar?')"><i class="fa fa-power-off"></i> Inativar</a>
                        <?php endif; ?>
                    </td>
                </tr>
                <?php endforeach; ?>
        ...
        <div class="pagination-container">
            <div class="pagination-info" id="paginationInfo">Mostrando 0 de 0 usuários</div>
...
function displayTable() {
    ...
    // 1. Primeiro, identificamos quais linhas passam pelo filtro da busca
    const filteredRows = allRows.filter(row => {
        // Se não houver busca, todas são elegíveis. 
        // Se houver, verificamos o atributo 'data-filtered'
        return row.getAttribute('data-filtered') !== 'false';
    });
    
    const totalPages = Math.ceil(filteredRows.length / rowsPerPage);
    
    // Ajuste de segurança: se a página atual for maior que o novo total de páginas
    if (currentPage > totalPages && totalPages > 0) currentPage = totalPages;

    const start = (currentPage - 1) * rowsPerPage;
    const end = start + rowsPerPage;

    // 2. Escondemos absolutamente todas as linhas primeiro
    allRows.forEach(row => row.style.display = "none");

    // 3. Mostramos apenas as linhas que pertencem à página atual dentro do grupo filtrado
...
function updatePaginationButtons(totalPages, totalActive) {
    ...
    info.innerText = `Mostrando página ${currentPage} de ${totalPages || 1} (${totalActive} usuários encontrados)`;
    ...
    prevBtn.onclick = () => { if(currentPage > 1) { currentPage--; displayTable(); } };
    ...
    nextBtn.onclick = () => { if(currentPage < totalPages) { currentPage++; displayTable(); } };
...
function filterUsers() {
    ...
    const table = document.getElementById("userTable");
    const rows = Array.from(table.getElementsByClassName("user-row"));

    rows.forEach(row => {
        const nameColumn = row.getElementsByClassName("user-name")[0];
        const emailColumn = row.getElementsByClassName("email-column")[0];
        const txtName = nameColumn ? (nameColumn.textContent || nameColumn.innerText) : "";
        const txtEmail = emailColumn ? (emailColumn.textContent || emailColumn.innerText) : "";
        
        // Se encontrar no nome ou no email, marca como visível para a paginação
    ...
    currentPage = 1; // Sempre volta para a página 1 ao filtrar
    displayTable();
}

// Inicializa a tabela assim que a página carregar
