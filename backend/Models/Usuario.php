...
class Usuario {
  ...
  function buscarUsuarios(){
    $sql = "SELECT * FROM s WHERE excluido_em IS NULL";
...
function buscarUsuariosAtivos() {
    $sql = "SELECT COUNT(*) AS total_ativos FROM s WHERE excluido_em IS NULL";
    $stmt = $this->db->prepare($sql);
    $stmt->execute();
    return $stmt->fetch(PDO::FETCH_COLUMN);
}
  ...
  public function paginacao(int $pagina = 1, int $porPagina = 50){
        ...
        $sql = "SELECT id_usuario, email_usuario, tipo_usuario, excluido_em FROM s 
        ...
        $totalStmt = $this->db->query("SELECT COUNT(*) FROM s");
  ...
  function buscarUsuariosPorEmail($email){
    $sql = "SELECT * FROM s WHERE email_usuario = :email AND excluido_em IS NULL";
    $stmt = $this->db->prepare($sql);
   ...
   $sql = "SELECT * FROM s where id_usuario = :id_usuario and excluido_em IS NULL";
   $stmt = $this->db->prepare($sql);
...
function buscarUsuariosPorEmailInativos($email){
   $sql = "SELECT * FROM s where email_usuario = :email and excluido_em IS NOT NULL";
   $stmt = $this->db->prepare($sql);
   $stmt->bindParam(':email', $email);
   $stmt->execute();
   return $stmt->fetchAll(PDO::FETCH_ASSOC);
...
VALUES (:nome, :email, :senha, :nivel, :foto)";
  ...
  public function buscarPorID($id){
        $sql = "SELECT * FROM s WHERE id_usuario = :id";
        $stmt = $this->db->prepare($sql);
  ...
  function deletarUsuario(int $id){
        ...
        $sql = "UPDATE s SET excluido_em = :excluido_em WHERE id_usuario = :id";
        $stmt = $this->db->prepare($sql);
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
